...
import { type ActionResult, errorMessage, fail, ok } from "@/lib/action-result";
...
export async function loginAction(
  ...
  try {
    ...
    return fail(errorMessage(err, "Unable to sign in."));
  }
}
...
export async function logoutAction(
  ...
  try {
    ...
    unstable_rethrow(err);
    return fail(errorMessage(err, "Unable to sign out."));
  }
}
...
export async function uploadJobAction(
  ...
  try {
    ...
    try {
      ...
      if (err instanceof QuotaExceededError) return fail(err.message);
      throw err;
    }
    ...
    unstable_rethrow(err);
    return fail(errorMessage(err, "Unable to submit print job."));
  }
}
...
export async function createUserAction(
  ...
  try {
    ...
    assertCsrf(csrf, formString(formData, "csrfToken"));
    if (admin.role !== UserRole.admin) return fail("Admin permission required.");
    ...
    if (!/^[A-Za-z0-9._-]{3,64}$/.test(username)) {
      return fail("Username must be 3–64 characters and use only letters, numbers, . _ -");
    }
    ...
    if (existing) return fail("Username already exists.");
    ...
    return ok(`User "${created.username}" created.`);
  } catch (err) {
    unstable_rethrow(err);
    return fail(errorMessage(err, "Unable to create user."));
  }
}
...
export async function updateUserAction(
  ...
  try {
    ...
    if (user.role === UserRole.admin && (role !== UserRole.admin || !isActive)) {
      ...
      if (activeAdmins <= 1) {
        return fail("Cannot disable or demote the last active admin.");
      }
    }
    ...
    if (resetPassword) {
      if (tempPassword.length < 8) {
        return fail("Temp password must be at least 8 characters.");
      }
    ...
    return ok(
      resetPassword
        ? `Updated ${user.username} and reset password.`
        : `Updated ${user.username}.`,
    );
  } catch (err) {
    unstable_rethrow(err);
    return fail(errorMessage(err, "Unable to update user."));
  }
}
...
export async function adminJobAction(
  ...
  try {
    ...
    try {
      ...
      if (err instanceof JobStateError) return fail(err.message);
      throw err;
    }
    ...
    revalidatePath("/admin/dashboard");
    revalidatePath("/dashboard");
    return ok(labels[action] ?? "Job updated.");
  } catch (err) {
    unstable_rethrow(err);
    return fail(errorMessage(err, "Unable to update job."));
  }
}

export async function cancelOwnJobAction(
  _prev: ActionResult | null,
  formData: FormData,
): Promise<ActionResult> {
  try {
    ...
    unstable_rethrow(err);
    return fail(errorMessage(err, "Unable to cancel job."));
  }
}

export async function updatePrintModeAction(
  _prev: ActionResult | null,
  formData: FormData,
): Promise<ActionResult> {
  try {
    ...
    revalidatePath("/dashboard");
    return ok(`Print mode set to ${printMode === "color" ? "color" : "black & white"}.`);
  } catch (err) {
    unstable_rethrow(err);
    return fail(errorMessage(err, "Unable to update print mode."));
  }
}
